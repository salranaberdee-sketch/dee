...
❌ ห้ามพัฒนาฟีเจอร์ใหม่ที่ทำให้ฟีเจอร์เดิมเสียหาย
```

---

## 🔄 กฎการตรวจสอบความขัดแย้งของฟีเจอร์ (FEATURE CONFLICT PREVENTION)

**⚠️ บังคับ: ฟีเจอร์ใหม่หรือการแก้ไขต้องไม่ทำให้ฟีเจอร์เดิมเสียหาย**

### หลักการ
1. ✅ **ตรวจสอบก่อนพัฒนา** - ต้องตรวจสอบว่าฟีเจอร์ใหม่จะกระทบฟีเจอร์เดิมหรือไม่
2. ✅ **ฟีเจอร์เดิมต้องทำงานได้** - ห้ามพัฒนาจนฟีเจอร์เดิมที่ใช้งานได้ดีกลับใช้ไม่ได้
3. ✅ **ถามก่อนเปลี่ยนแปลง** - หากพบว่าต้องเปลี่ยนแปลงฟีเจอร์เดิม ต้องแจ้งและขออนุมัติก่อน

### เมื่อพบความขัดแย้ง ต้อง:
```
1. 🔍 ระบุว่าฟีเจอร์ใหม่ขัดแย้งกับฟีเจอร์เดิมอย่างไร
2. 📋 อธิบายทางเลือกที่มี:
   - ทางเลือก A: ใช้ฟีเจอร์เดิม (ไม่เปลี่ยนแปลง)
   - ทางเลือก B: ใช้ฟีเจอร์ใหม่ (ต้องแก้ไขอะไรบ้าง)
   - ทางเลือก C: ผสมผสานทั้งสอง (ถ้าเป็นไปได้)
3. ⚠️ แจ้งผลกระทบของแต่ละทางเลือก
4. ✋ รอการตัดสินใจจาก user ก่อนดำเนินการ
```

### ตัวอย่าง
```
❌ ผิด: พัฒนาระบบ scoring ใหม่ แต่ทำให้ระบบ scoring เดิมใช้ไม่ได้
❌ ผิด: เปลี่ยน database schema โดยไม่ตรวจสอบว่ากระทบฟีเจอร์อื่น
❌ ผิด: แก้ไข component ที่ใช้ร่วมกันหลายที่โดยไม่ทดสอบทุกที่

✅ ถูก: ตรวจสอบ dependencies ก่อนแก้ไข
✅ ถูก: พบความขัดแย้ง → แจ้ง user → ให้เลือกทางเลือก → ดำเนินการ
✅ ถูก: ทดสอบฟีเจอร์เดิมหลังพัฒนาฟีเจอร์ใหม่เสร็จ
```

### Checklist ก่อนพัฒนาฟีเจอร์ใหม่
```
- [ ] ตรวจสอบว่าฟีเจอร์ใหม่ใช้ตาราง/component ร่วมกับฟีเจอร์เดิมหรือไม่
- [ ] ตรวจสอบว่าการเปลี่ยนแปลง schema กระทบ queries เดิมหรือไม่
- [ ] ตรวจสอบว่า UI changes กระทบ user flow เดิมหรือไม่
- [ ] ทดสอบฟีเจอร์เดิมที่เกี่ยวข้องหลังพัฒนาเสร็จ
```
